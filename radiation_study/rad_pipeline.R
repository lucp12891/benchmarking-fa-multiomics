...
run_factor_pipeline <- function(
  ...
  if ("MFA" %in% methods) {
    .stop_pkg("FactoMineR")
    if (exists("verbose") && isTRUE(verbose)) message("Running MFA ...")
    library(FactoMineR)
    
    # 1) transpose each view to samples x features
    Vt <- lapply(V, t)
    
    # 2) drop empty blocks (FactoMineR can't handle group size 0)
    keep_block <- vapply(Vt, ncol, 1L) > 0L
    Vt <- Vt[keep_block]
    if (!length(Vt)) {
      # no variables at all -> return empty results for MFA
      rn_scores <- rownames(V[[1L]])
      if (is.null(rn_scores)) rn_scores <- paste0("sample_", seq_len(nrow(V[[1L]])))
      scores_empty <- matrix(numeric(0), nrow = length(rn_scores), ncol = 0,
                             dimnames = list(rn_scores, NULL))
      loadings_empty <- matrix(numeric(0), nrow = 0, ncol = 0,
                               dimnames = list(character(0), NULL))
      results$MFA <- list(scores = scores_empty, loadings = loadings_empty,
                          meta = list(n_factors = 0L))
    } else {
      
      # 3) tiny jitter for any zero-variance columns (avoid internal drops)
      jitter_cols <- function(M, eps = 1e-8) {
        if (!ncol(M)) return(M)
        sds <- apply(M, 2, sd, na.rm = TRUE)
        bad <- which(!is.finite(sds) | sds == 0)
      ...
      # 4) bind and build group sizes (all > 0 now)
      X_mfa <- do.call(cbind, Vt)
      grp_sizes <- vapply(Vt, ncol, 1L)
      
      # 5) prepare data.frame for FactoMineR
      X_df <- as.data.frame(X_mfa)
      if (!is.null(rownames(X_mfa))) rownames(X_df) <- rownames(X_mfa)
      
      # 6) if k<=0, SKIP MFA and return empty factors with correct dims
      if (is.null(k)) k <- 0L
      if (k <= 0L) {
        # scores: n_samples x 0
        scores <- matrix(numeric(0), nrow = nrow(X_df), ncol = 0,
                         dimnames = list(rownames(X_df), NULL))
        
        # loadings rownames with "block::feature"
        rn_load <- unlist(
          mapply(function(block, nm) paste0(nm, "::", colnames(block)),
                 Vt, names(Vt), SIMPLIFY = FALSE),
          use.names = FALSE
        )
        loadings <- matrix(numeric(0), nrow = length(rn_load), ncol = 0,
                           dimnames = list(rn_load, NULL))
        
        results$MFA <- list(scores = scores, loadings = loadings,
                            meta = list(n_factors = 0L))
        
      } else {
        # 7) run MFA with a safe ncp (>=1 and within admissible rank)
        max_rank <- min(nrow(X_df) - 1L, sum(grp_sizes) - length(grp_sizes))
        ncp_eff  <- max(1L, min(k, max_rank))
        
        fit <- FactoMineR::MFA(
          X_df,
          group = grp_sizes,
          type  = rep("c", length(grp_sizes)),
          ncp   = ncp_eff,
          graph = FALSE
        )
        
        # 8) SCORES (limit to requested k if needed)
        Z <- fit$ind$coord
        if (is.null(Z)) {
          Z <- matrix(0, nrow = nrow(X_df), ncol = 0,
                      dimnames = list(rownames(X_df), NULL))
          ...
          Z <- as.matrix(Z)
          if (ncol(Z) > k) Z <- Z[, seq_len(k), drop = FALSE]
          colnames(Z) <- paste0("F", seq_len(ncol(Z)))
        }
        
        # 9) LOADINGS (prefer $var$coord, fall back to $quanti.var$coord)
        L <- fit$var$coord
        if (is.null(L)) L <- fit$quanti.var$coord
        if (!is.null(L)) {
          dim_cols <- grep("^Dim\\.", colnames(L))
          L <- as.matrix(L[, dim_cols, drop = FALSE])
          if (ncol(L) > ncol(Z)) L <- L[, seq_len(ncol(Z)), drop = FALSE]
          colnames(L) <- colnames(Z)
          
          block_names <- rep(names(Vt), times = grp_sizes)
          rownames(L) <- paste0(block_names, "::", colnames(X_mfa))
        } else {
          rn_load <- unlist(
            mapply(function(block, nm) paste0(nm, "::", colnames(block)),
          ...
          L <- matrix(0, nrow = length(rn_load), ncol = ncol(Z),
                      dimnames = list(rn_load, colnames(Z)))
        }
...
#===========================================
set.seed(1234)
benchmark_radiation <- run_factor_pipeline(
  list(mRNA = as.matrix(mRNA), proteomics = as.matrix(proteomics)),
  methods = c("FABIA","MOFA","GFA","MFA"),
  k = 1,
  scale_features = TRUE
)
...
becnhmark_rad_merged <- attach_scores_metadata(benchmark_radiation, rad_metadata, sample_col = "sample")
...
scores_df_for_method <- function(bench, method, factor_idx = 1, group_col = "Treatment") {
  ...
  grp_chr <- dplyr::case_when(
    is.na(grp_raw) ~ "NA",
    as.character(grp_raw) %in% c("TRUE","FALSE") ~ as.character(grp_raw),
    TRUE ~ "NA"
  )
...
p_none <- plot_scores_scatter_grid(
  becnhmark_rad_merged, factor_idx = 3, group_col = "Condition",
  methods = c("MOFA","FABIA","MFA","GFA"),
  panel_tags = NULL,
  tag_position = "none"
)
...
# Scores
fabia_score_rad_df <- data.frame(becnhmark_rad_merged$FABIA$scores); fabia_score_rad_df$sample <- rownames(fabia_score_rad_df)
# rename columns that start with "F"
colnames(fabia_score_rad_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_score_rad_df))

mofa_score_rad_df <- data.frame(becnhmark_rad_merged$MOFA$scores); mofa_score_rad_df$sample <- rownames(mofa_score_rad_df)
# rename columns that start with "F"
colnames(mofa_score_rad_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_score_rad_df))

mfa_score_rad_df <- data.frame(becnhmark_rad_merged$MFA$scores); mfa_score_rad_df$sample <- rownames(mfa_score_rad_df)
# rename columns that start with "F"
colnames(mfa_score_rad_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_score_rad_df))

gfa_score_rad_df <- data.frame(becnhmark_rad_merged$GFA$scores); gfa_score_rad_df$sample <- rownames(gfa_score_rad_df)
# rename columns that start with "F"
colnames(gfa_score_rad_df) <- sub("^F(\\d+)$", "F\\1 (GFA)", colnames(gfa_score_rad_df))

names(fabia_score_rad_df)
names(mofa_score_rad_df)
names(mfa_score_rad_df)
names(gfa_score_rad_df)
...
# LOADINGS
fabia_loading_rad_df <- data.frame(becnhmark_rad_merged$FABIA$loading); fabia_loading_rad_df$feature <- rownames(fabia_loading_rad_df)
# rename columns that start with "F"
colnames(fabia_loading_rad_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_loading_rad_df))

mofa_loading_rad_df <- data.frame(becnhmark_rad_merged$MOFA$loading); mofa_loading_rad_df$feature <- rownames(mofa_loading_rad_df)
# rename columns that start with "F"
colnames(mofa_loading_rad_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_loading_rad_df))

mfa_loading_rad_df <- data.frame(becnhmark_rad_merged$MFA$loading); mfa_loading_rad_df$feature <- rownames(mfa_loading_rad_df)
# rename columns that start with "F"
colnames(mfa_loading_rad_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_loading_rad_df))

gfa_loading_rad_df <- data.frame(becnhmark_rad_merged$GFA$loading); gfa_loading_rad_df$feature <- rownames(gfa_loading_rad_df)
# rename columns that start with "F"
colnames(gfa_loading_rad_df) <- sub("^F(\\d+)$", "F\\1 (GFA)", colnames(gfa_loading_rad_df))

names(fabia_loading_rad_df)
names(mofa_loading_rad_df)
names(mfa_loading_rad_df)
names(gfa_loading_rad_df)


# Helper function: split loading df by prefix
split_by_prefix <- function(df, prefix_s16 = "^s16", prefix_met = "^met", prefix_lip = "^lip") {
...
fabia_rad_split <- split_by_prefix(fabia_loading_rad_df)
mofa_rad_split  <- split_by_prefix(mofa_loading_rad_df)
mfa_rad_split   <- split_by_prefix(mfa_loading_rad_df)
gfa_rad_split   <- split_by_prefix(gfa_loading_rad_df)
...
bench_rad_res <- benchmark_factor_methods(becnhmark_rad_merged)
