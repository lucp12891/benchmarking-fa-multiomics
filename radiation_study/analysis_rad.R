...
#===========================================
set.seed(1234)
benchmark_rad <- run_factor_pipeline(
  list(mRNA = as.matrix(mRNA), prot = as.matrix(proteomics)),
  methods = c("FABIA","MOFA","MFA","GFA"),
  k = 1,
  scale_features = TRUE)

# ============================================================
# merged <- merge_scores_with_metadata(benchmark_rad, rad_metadata, sample_col = "sample")
# ============================================================
benchmark_rad_meta <- attach_scores_metadata(benchmark_rad, rad_metadata, sample_col = "sample")

#============================================
# Create score scatter plot by Condition
#============================================
p_none <- plot_scores_scatter_grid(
  benchmark_rad_meta, factor_idx = 1, group_col = "Condition",
...
# Scores
fabia_score_df <- data.frame(benchmark_rad$FABIA$scores); fabia_score_df$sample <- rownames(fabia_score_df)
# rename columns that start with "F"
colnames(fabia_score_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_score_df))

mofa_score_df <- data.frame(benchmark_rad$MOFA$scores); mofa_score_df$sample <- rownames(mofa_score_df)
# rename columns that start with "F"
colnames(mofa_score_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_score_df))

mfa_score_df <- data.frame(benchmark_rad$MFA$scores); mfa_score_df$sample <- rownames(mfa_score_df)
# rename columns that start with "F"
colnames(mfa_score_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_score_df))

gfa_score_df <- data.frame(benchmark_rad$GFA$scores); gfa_score_df$sample <- rownames(gfa_score_df)
# rename columns that start with "F"
...
#============================================
# LOADINGS
fabia_loading_df <- data.frame(benchmark_rad$FABIA$loading); fabia_loading_df$feature <- rownames(fabia_loading_df)
# rename columns that start with "F"
colnames(fabia_loading_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_loading_df))

mofa_loading_df <- data.frame(benchmark_rad$MOFA$loading); mofa_loading_df$feature <- rownames(mofa_loading_df)
# rename columns that start with "F"
colnames(mofa_loading_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_loading_df))

mfa_loading_df <- data.frame(benchmark_rad$MFA$loading); mfa_loading_df$feature <- rownames(mfa_loading_df)
# rename columns that start with "F"
colnames(mfa_loading_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_loading_df))

gfa_loading_df <- data.frame(benchmark_rad$GFA$loading); gfa_loading_df$feature <- rownames(gfa_loading_df)
# rename columns that start with "F"
...
# ============================================================
bench_radx <- benchmark_factor_methods(benchmark_rad_meta)
