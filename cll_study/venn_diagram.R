...
x4 <- list(
  A = which(score_tbl_index$F1_FABIA_index == 1), 
  B = which(score_tbl_index$F1_GFA_index == 1),
  C = which(score_tbl_index$F1_MOFA_index == 1),
  D = which(score_tbl_index$F1_MFA_index == 1)
)

m <- build_comb_mat(x4)                    # default mode = "distinct"
