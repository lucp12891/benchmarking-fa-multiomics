...
run_factor_pipeline_str <- function(
    X,
  ...
  if ("FABIA" %in% methods) {
    ...
    set.seed(21)#991489
    fit <- fabia(X = M_fabia, p = p, alpha = 0.02, cyc = 1000, spz = 0.1, spl = 0.2, lap = 1.0, nL = 2)
...
#===========================================
set.seed(1234)
benchmark_res_fabcut <- run_factor_pipeline_str(
...
subset_all_methods_by_fabia <- function(results, tol = 0, fabia_name = "FABIA",
                                        columns = "all") {
  stopifnot(is.list(results))
  if (!fabia_name %in% names(results)) stop("FABIA entry not found in results.")
  if (is.null(results[[fabia_name]]$loadings)) stop("results$FABIA$loadings is NULL.")
  
  Lf <- results[[fabia_name]]$loadings
  if (!is.matrix(Lf)) Lf <- as.matrix(Lf)
  
  # ---- choose which factor columns to use for the FABIA keep-mask ----
  cols <- columns
  if (is.character(cols)) {
    if (length(cols) == 1 && tolower(cols) == "all") {
      cols <- seq_len(ncol(Lf))
    } else if (all(grepl("^F[0-9]+$", cols, ignore.case = TRUE))) {
      cols <- as.integer(sub("^F", "", toupper(cols)))
    } else {
      # allow passing actual colnames if they exist
      if (!is.null(colnames(Lf)) && all(cols %in% colnames(Lf))) {
        cols <- match(cols, colnames(Lf))
      } else {
        stop("`columns` must be 'all', numeric indices (e.g. 1), 'F1'/'F2', or valid column names in FABIA loadings.")
      }
    }
  }
  
  if (!is.numeric(cols)) stop("`columns` could not be resolved to numeric indices.")
  cols <- unique(as.integer(cols))
  if (any(cols < 1 | cols > ncol(Lf))) stop("`columns` index out of bounds for FABIA loadings.")
  
  # Keep a feature if it is non-zero in any of the selected columns (within tolerance)
  keep <- apply(abs(Lf[, cols, drop = FALSE]) > tol, 1, any)
  keep_idx <- which(keep)
  
  # Optional alignment checks using rownames (recommended)
  ref_rn <- rownames(Lf)
  have_rn <- !is.null(ref_rn)
  
  # Apply to each method
  out <- results
  dim_report <- data.frame(
    method = names(results),
    before_rows = NA_integer_,
    before_cols = NA_integer_,
    after_rows  = NA_integer_,
    after_cols  = NA_integer_
  )
  
  for (m in names(results)) {
    if (is.null(results[[m]]$loadings)) next
    Lm <- results[[m]]$loadings
    if (!is.matrix(Lm)) Lm <- as.matrix(Lm)
    
    dim_report[dim_report$method == m, c("before_rows","before_cols")] <- dim(Lm)
    
    if (have_rn && !is.null(rownames(Lm))) {
      common <- intersect(ref_rn[keep], rownames(Lm))
      if (length(common) == 0) {
        stop(sprintf("No overlapping feature names between FABIA and %s loadings.", m))
      }
      out[[m]]$loadings <- Lm[common, , drop = FALSE]
    } else {
      out[[m]]$loadings <- Lm[keep_idx, , drop = FALSE]
    }
    
    dim_report[dim_report$method == m, c("after_rows","after_cols")] <- dim(out[[m]]$loadings)
  }
  ...
  summary <- list(
    ...
    n_features_total = nrow(Lf),
    n_features_kept  = sum(keep),
    n_features_removed = sum(!keep),
    prop_kept = sum(keep) / nrow(Lf),
    dim_report = dim_report,
    kept_features = if (have_rn) ref_rn[keep] else keep_idx
  )
  
  list(results = out, summary = summary)
}

res_sub <- subset_all_methods_by_fabia(benchmark_res_fabcut, tol = 0, columns = "F1")

# Updated results list (same structure, but loadings filtered)
benchmark_res_fabcut_filtered <- res_sub$results
...
mergedx_fabcut_sub <- attach_scores_metadata(benchmark_res_fabcut_filtered, CLL_metadata, sample_col = "sample")
...
p_nonex <- plot_scores_scatter_grid(
  mergedx_fabcut_sub, factor_idx = 1, group_col = "IGHV",
  methods = c("MOFA","FABIA","MFA","GFA"),
  panel_tags = NULL,
  tag_position = "none"
)
...
#============================================
benchmark_res_fabcut_sub = mergedx_fabcut_subx
# Scores
fabia_score_fabcut_df <- data.frame(benchmark_res_fabcut_sub$FABIA$scores); fabia_score_fabcut_df$sample <- rownames(fabia_score_fabcut_df)
# rename columns that start with "F"
colnames(fabia_score_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_score_fabcut_df))

mofa_score_fabcut_df <- data.frame(benchmark_res_fabcut_sub$MOFA$scores); mofa_score_fabcut_df$sample <- rownames(mofa_score_fabcut_df)
# rename columns that start with "F"
colnames(mofa_score_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_score_fabcut_df))

mfa_score_fabcut_df <- data.frame(benchmark_res_fabcut_sub$MFA$scores); mfa_score_fabcut_df$sample <- rownames(mfa_score_fabcut_df)
# rename columns that start with "F"
colnames(mfa_score_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_score_fabcut_df))

gfa_score_fabcut_df <- data.frame(benchmark_res_fabcut_sub$GFA$scores); gfa_score_fabcut_df$sample <- rownames(gfa_score_fabcut_df)
# rename columns that start with "F"
colnames(gfa_score_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (GFA)", colnames(gfa_score_fabcut_df))

names(fabia_score_fabcut_df)
names(mofa_score_fabcut_df)
names(mfa_score_fabcut_df)
names(gfa_score_fabcut_df)
...
# LOADINGS
fabia_loading_fabcut_df <- data.frame(benchmark_res_fabcut_sub$FABIA$loading); fabia_loading_fabcut_df$feature <- rownames(fabia_loading_fabcut_df)
# rename columns that start with "F"
colnames(fabia_loading_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (FABIA)", colnames(fabia_loading_fabcut_df))

mofa_loading_fabcut_df <- data.frame(benchmark_res_fabcut_sub$MOFA$loading); mofa_loading_fabcut_df$feature <- rownames(mofa_loading_fabcut_df)
# rename columns that start with "F"
colnames(mofa_loading_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (MOFA)", colnames(mofa_loading_fabcut_df))

mfa_loading_fabcut_df <- data.frame(benchmark_res_fabcut_sub$MFA$loading); mfa_loading_fabcut_df$feature <- rownames(mfa_loading_fabcut_df)
# rename columns that start with "F"
colnames(mfa_loading_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (MFA)", colnames(mfa_loading_fabcut_df))

gfa_loading_fabcut_df <- data.frame(benchmark_res_fabcut_sub$GFA$loading); gfa_loading_fabcut_df$feature <- rownames(gfa_loading_fabcut_df)
# rename columns that start with "F"
colnames(gfa_loading_fabcut_df) <- sub("^F(\\d+)$", "F\\1 (GFA)", colnames(gfa_loading_fabcut_df))

names(fabia_loading_fabcut_df)
names(mofa_loading_fabcut_df)
names(mfa_loading_fabcut_df)
names(gfa_loading_fabcut_df)
...
# Apply to each metho
fabia_split_fabcut <- split_by_pattern(
  df = fabia_loading_fabcut_df,
  column = "feature",
  patterns = patterns
)

mofa_split_fabcut <- split_by_pattern(
  df = mofa_loading_fabcut_df,
  column = "feature",
  patterns = patterns
)

mfa_split_fabcut <- split_by_pattern(
  df = mfa_loading_fabcut_df,
  column = "feature",
  patterns = patterns
)

gfa_split_fabcut <- split_by_pattern(
  df = gfa_loading_fabcut_df,
  column = "feature",
  patterns = patterns
)
...
benchmark_factor_methods <- function(
  ...
  if (length(available) < 2)
    stop("Need at least two methods in 'benchmark_res_fabcut_sub'. Found: ", paste(available, collapse=", "))
...
benchmark_factor_methods <- function(
  ...
  # ---- 2) Helpers for per-dataset loadings --------------------------------
  get_prefix <- function(rn) sub("::.*$", "", rn)       # "rna", "prot", ...
  get_suffix <- function(rn) sub("^.*::", "", rn)       # "feature_id", ...
  normalize_ids <- function(xnames) {
    if (is.null(xnames)) return(NULL)
    if (is.null(feature_id_split)) return(xnames)
    parts <- strsplit(xnames, split = feature_id_split, fixed = TRUE)
  ...
  # ---- plotting helper (scatter + r), SAFE: only add lm line if identifiable ----
  make_scatter <- function(x, y, title, xlab, ylab) {
    df <- data.frame(x = as.numeric(x), y = as.numeric(y))
    df <- df[is.finite(df$x) & is.finite(df$y), , drop = FALSE]
    
    r <- if (nrow(df) >= 3 && stats::sd(df$x) > 0 && stats::sd(df$y) > 0)
      suppressWarnings(stats::cor(df$x, df$y, use = "pairwise.complete.obs"))
    else NA_real_
    
    p <- ggplot2::ggplot(df, ggplot2::aes(x = x, y = y)) +
      ggplot2::geom_point(alpha = 0.6, size = 1) +
      ggplot2::annotate("text", x = Inf, y = Inf,
                        label = if (is.finite(r)) sprintf("r = %.3f", r) else "r = NA",
                        hjust = 1.05, vjust = 1.5, size = 4) +
      ggplot2::labs(title = title, x = xlab, y = ylab) +
      ggplot2::theme_minimal(base_size = plot_theme_base_size) +
      ggplot2::theme(plot.title = ggplot2::element_text(face = "bold"))
    
    if (nrow(df) >= 3 && length(unique(df$x)) >= 2 && length(unique(df$y)) >= 2) {
  ...
  .align_sign_vec <- function(ref_vec, v) {
    ref_vec <- as.numeric(ref_vec); v <- as.numeric(v)
    ok <- is.finite(ref_vec) & is.finite(v)
    if (sum(ok) < 3) return(v)
    c1 <- suppressWarnings(stats::cor(ref_vec[ok], v[ok]))
    if (!is.finite(c1)) return(v)
    if (c1 < 0) -v else v
  }
  
  # ---- align method factors to reference (scores) ----
  .align_to_reference_scores <- function(scores_list, ref_method, match_factors) {
    out <- list()
    ref_k <- ncol(scores_list[[ref_method]])
    for (m in names(scores_list)) {
      if (m == ref_method) next
      common <- intersect(rownames(scores_list[[ref_method]]), rownames(scores_list[[m]]))
      A <- scores_list[[ref_method]][common,,drop=FALSE]
      B <- scores_list[[m]][common,,drop=FALSE]
      C <- stats::cor(A, B, use = "pairwise.complete.obs")
      ms <- match_factors(C)
      map <- rep(NA_integer_, ref_k)
      if (!is.null(ms) && nrow(ms)) {
        for (k in seq_len(nrow(ms))) map[ms$A[k]] <- ms$B[k]
      }
      out[[m]] <- map
    }
    out
  ...
  # ---- align method factors to reference (loadings per dataset) ----
  .align_to_reference_loadings <- function(loadings_list, ref_method, get_prefix, get_suffix, match_factors) {
    out <- list()
    pref_ref <- get_prefix(rownames(loadings_list[[ref_method]]))
    datasets_ref <- unique(pref_ref)
    ...
    for (om in datasets_ref) {
      ...
      idx_ref <- which(pref_ref == om)
      ids_ref <- get_suffix(rownames(loadings_list[[ref_method]])[idx_ref])
      A_all <- loadings_list[[ref_method]][idx_ref,,drop=FALSE]
      
      for (m in names(loadings_list)) {
        if (m == ref_method) next
        pref_m <- get_prefix(rownames(loadings_list[[m]]))
        idx_m <- which(pref_m == om)
        if (!length(idx_m)) next
        ids_m <- get_suffix(rownames(loadings_list[[m]])[idx_m])
        common <- intersect(ids_ref, ids_m)
        if (!length(common)) next
        
        A <- A_all[match(common, ids_ref), , drop=FALSE]
        B <- loadings_list[[m]][idx_m[match(common, ids_m)], , drop=FALSE]
  ...
  # ======================== SAFE pairs() plotting =========================
  # .make_pairs_plot <- function(mat, main = NULL) {
  #   mat <- as.data.frame(mat, check.names = FALSE)
  #   if (ncol(mat) < 2) return(NULL)
  #   
  #   # coerce to numeric matrix for pairs
  #   for (j in seq_len(ncol(mat))) mat[[j]] <- suppressWarnings(as.numeric(mat[[j]]))
  #   
  ...
  #     par(usr = c(usr[1:2], 0, 1.5))
  #     h <- hist(x, plot = FALSE)
  #     if (!length(h$breaks) || all(h$counts == 0)) return(invisible(NULL))
  #     y <- h$counts / max(h$counts)
  #     rect(h$breaks[-length(h$breaks)], 0, h$breaks[-1], y, col = "grey85", border = "grey85")
  #   }
  #   
  #   panel.cor.safe <- function(x, y, digits = 2, ...) {
  #     usr <- par("usr"); on.exit(par(usr))
  #     par(usr = c(0, 1, 0, 1))
  #     ok <- is.finite(x) & is.finite(y)
  #     x <- x[ok]; y <- y[ok]
  #     r <- if (length(x) >= 3 && stats::sd(x) > 0 && stats::sd(y) > 0)
  ...
  #   }
  #   
  #   panel.lm.safe <- function(x, y, ...) {
  #     points(x, y, pch = 1, cex = 0.6)
  #     ok <- is.finite(x) & is.finite(y)
  #     x <- x[ok]; y <- y[ok]
  #     if (length(x) < 3) return(invisible(NULL))
  #     if (length(unique(x)) < 2 || length(unique(y)) < 2) return(invisible(NULL))
  #     fit <- suppressWarnings(stats::lm(y ~ x))
  #     cf <- coef(fit)
  #     if (length(cf) == 2 && all(is.finite(cf))) {
  #       abline(a = cf[1], b = cf[2], lty = 2, lwd = 1)
  #     }
  #   }
  #   
  #   graphics::pairs(
  #     mat,
  #     main = main,
  #     lower.panel = panel.lm.safe,
  #     upper.panel = panel.cor.safe,
  #     diag.panel  = panel.hist
  #   )
  # }
  .make_pairs_plot <- function(mat, main = NULL, txt_cex = 1.6, digits = 2, n_bins = 50) {
    mat <- as.data.frame(mat, check.names = FALSE)
    if (ncol(mat) < 2) return(NULL)
    
    # Rename columns to method name only
    cn <- colnames(mat)
    cn <- gsub("^Score\\s*\\(([^)]+)\\)$", "\\1", cn)
    cn <- gsub("^Loadings?\\s*\\(([^)]+)\\)$", "\\1", cn)
    colnames(mat) <- cn
    
    # numeric coercion
    for (j in seq_len(ncol(mat))) mat[[j]] <- suppressWarnings(as.numeric(mat[[j]]))
    
    # Diagonal histogram (NO density line) + more bins
    panel.hist <- function(x, ...) {
      x <- x[is.finite(x)]
      if (!length(x)) return(invisible(NULL))
      usr <- par("usr"); on.exit(par(usr))
      par(usr = c(usr[1:2], 0, 1.5))
      h <- hist(x, plot = FALSE, breaks = n_bins)
      if (!length(h$breaks) || all(h$counts == 0)) return(invisible(NULL))
      y <- h$counts / max(h$counts)
    ...
    # Upper: correlation only (no 'r ='), bold, big
    panel.cor.safe <- function(x, y, ...) {
      usr <- par("usr"); on.exit(par(usr))
      par(usr = c(0, 1, 0, 1))
      ok <- is.finite(x) & is.finite(y)
      x <- x[ok]; y <- y[ok]
      ...
      txt <- if (is.finite(r)) sprintf(paste0("%.", digits, "f"), r) else "NA"
      text(0.5, 0.5, txt, cex = txt_cex, font = 2)
    }
    
    # Lower: points + lm line only if identifiable
    panel.lm.safe <- function(x, y, ...) {
      points(x, y, pch = 1, cex = 0.6)
      ok <- is.finite(x) & is.finite(y)
      x <- x[ok]; y <- y[ok]
      ...
      cf <- coef(fit)
      if (length(cf) == 2 && all(is.finite(cf))) {
        abline(a = cf[1], b = cf[2], lty = 2, lwd = 1, col = "red")
      }
    }
    
    # Diagonal: method name + histogram
    i_diag <- 0L
    panel.diag.name <- function(x, ...) {
      i_diag <<- i_diag + 1L
      usr <- par("usr"); on.exit(par(usr))
      par(usr = c(0, 1, 0, 1))
      lab <- colnames(mat)[i_diag]
      text(0.5, 0.88, lab, cex = txt_cex, font = 2)
      panel.hist(x, ...)
    }
  ...
  pair_heatmaps_scores    <- list()
  pair_heatmaps_loadings  <- list()
  pair_scatter_scores     <- list()
  pair_scatter_loadings   <- list()
  ...
  for (om in datasets_ref) {
    ...
    for (i in seq_len(ref_k)) {
      cols <- list()
      ref_vec <- loadings_list[[ref_method]][row_idx[[ref_method]], i]
      
      for (m in available) {
        if (m == ref_method) {
          cols[[paste0(m)]] <- ref_vec
        } else {
          if (is.null(maps_loadings[[om]]) || is.null(maps_loadings[[om]][[m]])) next
          j <- maps_loadings[[om]][[m]][i]
          if (is.na(j)) next
          v <- loadings_list[[m]][row_idx[[m]], j]
          v <- .align_sign_vec(ref_vec, v)
      ...
      if (length(cols) >= 2) {
        mat <- as.data.frame(cols, check.names = FALSE)
        lab <- paste0("Loadings (", om, ") — Pearson cor.")
        pairs_loadings_by_dataset_and_factor[[om]][[paste0("F", i)]] <-
          .make_pairs_plot(mat, main = lab)
      }
    }
  }
  ...
  # ---- 7) Optional AUCs vs truth (unchanged, but stable) ----
  normalize_ids_suffix <- function(xnames) normalize_ids(xnames)
  auc_results <- NULL
  if (!is.null(ground_truth_feature) || !is.null(ground_truth_sample)) {
    if (!requireNamespace("pROC", quietly = TRUE)) stop("To compute AUC, please install 'pROC'.")
    calc_auc <- function(scores, truth) {
      truth <- as.numeric(truth)
      if (!length(scores) || !length(truth) || length(scores) != length(truth)) return(NA_real_)
      if (stats::sd(scores, na.rm = TRUE) == 0) return(NA_real_)  # degenerate predictor
      roc <- pROC::roc(response = truth, predictor = scores, quiet = TRUE, direction = "<")
      as.numeric(pROC::auc(roc))
    }
    rows_auc <- list()
  ...
  list(
    ...
    loadings                 = loadings_list,
    pairwise_scores          = pair_summaries_scores,
    pairwise_loadings        = pair_summaries_loadings,
    pair_heatmaps_scores     = pair_heatmaps_scores,
    pair_heatmaps_loadings   = pair_heatmaps_loadings,
    pair_scatter_scores      = pair_scatter_scores,
    pair_scatter_loadings    = pair_scatter_loadings,
    all_scores_heatmap       = p_all,
    summary_table_scores     = summary_table_scores,
    summary_table_loadings   = summary_table_loadings,
    auc_results              = auc_results,
    pairs_scores_by_factor               = pairs_scores_by_factor,
    pairs_loadings_by_dataset_and_factor = pairs_loadings_by_dataset_and_factor
  )
}

benchx_fabcut_sub <- benchmark_factor_methods(benchmark_res_fabcut_sub)
