...
func_mfa <- function(data, num) {
  ...
  indices_features.1a <- unlist(simulated_data[[paste0("iteration_", i)]]$indices_features.1[1])
  indices_features.1b <- unlist(simulated_data[[paste0("iteration_", i)]]$indices_features.1[2])
  ...
  # Load signal annotations
  indices_features.2a <- unlist(simulated_data[[paste0("iteration_", i)]]$indices_features.2[1])
  ...
  indices_samples.a <- unlist(simulated_data[[paste0("iteration_", i)]]$indices_samples[1])
  indices_samples.b <- unlist(simulated_data[[paste0("iteration_", i)]]$indices_samples[2])
...
varphi.five_multisim <- function(loading_or_score, omic = NULL, factor = NULL, type = c("loading", "score"), method = NULL) {
  type <- match.arg(type)
  numeric_values <- as.numeric(unlist(loading_or_score))
  
  if (is.null(method) || method != "multiple.factor") {
    stop("Invalid or missing 'method' argument. Must be 'multiple.factor'.")
  }
