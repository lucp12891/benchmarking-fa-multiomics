...
#   numeric_values <- as.numeric(unlist(loading_or_score))
#   
#   # Skip if values are all NA or too few distinct values
#   if (all(is.na(numeric_values)) || length(unique(numeric_values)) < 2) {
#     warning("Too few distinct values in '", vector_name, "' for k-means. Returning Inf as threshold.")
#     return(Inf)
#   }
#   
#   if (any(grepl("loading", vector_name)) || any(grepl("score", vector_name))) {
#     clusters <- kmeans(numeric_values, centers = 2)$cluster
#     signal_cluster <- which.max(tapply(numeric_values, clusters, mean))
#     varphi <- mean(numeric_values[clusters == signal_cluster], na.rm = TRUE)
#     return(varphi)
#   }
#   
#   warning("Variable name does not contain 'loading' or 'score'. Returning Inf.")
#   return(Inf)
# }
...
varphi.ten <- function(loading_or_score) {
  ...
  if (all(is.na(numeric_values)) || length(unique(numeric_values)) < 2) {
    warning("Too few distinct values in '", vector_name, "'. Returning Inf as cutoff.")
    return(Inf)
  }
  
  # Run k-means clustering
  km <- tryCatch(
    kmeans(numeric_values, centers = 2),
    error = function(e) {
      warning("kmeans failed for '", vector_name, "': ", e$message)
      return(NULL)
    }
  )
  
  if (is.null(km)) return(Inf)
  
  # Identify the signal cluster (higher mean)
  cluster_means <- tapply(numeric_values, km$cluster, mean)
  signal_cluster <- which.max(cluster_means)
  
  # Return mean of values in the signal cluster
  varphi <- mean(numeric_values[km$cluster == signal_cluster], na.rm = TRUE)
  return(varphi)
}
...
# Get sigma from command line argument
#args <- commandArgs(trailingOnly = TRUE)
#if (length(args) == 0) stop("Please provide a sigma value.")
sigma <- 6#as.numeric(args[1])

# Fix seed for reproducibility
set.seed(5121)

cat("Running simulate_MOmicsR for sigma =", sigma, "\n")
...
# Save output to HPC location
saveRDS(
  simulation_result,
  file = paste0("/user/leuven/364/vsc36498/sim_results/sim_MomicResults_multifactor_", sigma, ".rds")
)

cat("Completed and saved results for sigma =", sigma, "\n")
