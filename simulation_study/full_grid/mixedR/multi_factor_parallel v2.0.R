...
func_gfa <- function(data, num) {
  ...
  var_false <- var(gfa_omic_one_load$loading_GFA1[gfa_omic_one_load$signal_a == FALSE & gfa_omic_one_load$signal_b == FALSE], na.rm = TRUE)
  #mean_false <- mean(gfa_omic_one_load$loading_GFA1[gfa_omic_one_load$signal_b == FALSE], na.rm = TRUE)
  mean_false <- mean(gfa_omic_one_load$loading_GFA1[gfa_omic_one_load$signal_a == FALSE & gfa_omic_one_load$signal_b == FALSE], na.rm = TRUE)
  
  # Suppress elevated factor
  gfa_omic_one_load$loading_GFA1 <- ifelse(
    gfa_omic_one_load$signal_b == TRUE,
    rnorm(length(gfa_omic_one_load$loading_GFA1[gfa_omic_one_load$signal_b == TRUE]), mean_false, var_false), # Action if TRUE
  ...
  #var_false <- var(gfa_omic_one_load$loading_GFA2[gfa_omic_one_load$signal_a == FALSE], na.rm = TRUE)
  var_false <- var(gfa_omic_one_load$loading_GFA2[gfa_omic_one_load$signal_a == FALSE & gfa_omic_one_load$signal_b == FALSE], na.rm = TRUE)
  #mean_false <- mean(gfa_omic_one_load$loading_GFA2[gfa_omic_one_load$signal_a == FALSE], na.rm = TRUE)
  mean_false <- mean(gfa_omic_one_load$loading_GFA2[gfa_omic_one_load$signal_a == FALSE & gfa_omic_one_load$signal_b == FALSE], na.rm = TRUE)
  ...
  weights_composite <- list(omic.one_weights = gfa_omic_one_load, 
                            omic.two_weights = load_omic_two, 
...
ssim_MOmicR <- function(n_features_one, n_features_two, n_samples, var_sigma, num_biclusters = 2, num_iterations, method) {
  ...
  for (sigma in var_sigma) {
    ...
    varphi_functions <- list(
      ...
      varphi.five = varphi.five_multisim, # prop of true signal
      ...
      for (i in 1:length(simulated_data)) {
        try({
          ...
          loading_data.not.scaled.omic.two.f1 <- data.frame(
            ...
            loading_MOFA1 = mofa_result[["weights"]][["omic.two_weights"]][["loading_MOFA2"]],
          ...
          score_data.not.scaled1 <- data.frame(
            ...
            score_MOFA1 = mofa_result[["scores"]][["scores"]][["score_MOFA2"]],
            score_MFA1 = mfa_result[["scores"]][["factor_scores"]][["score_MFA1"]],
            score_GFA1 = gfa_result[["scores"]][["factor_scores"]][["score_GFA1"]]
          )
          ...
          score_data.not.scaled2 <- data.frame(
            sample = mofa_result[["scores"]][["scores"]][["sample"]],
            score_FABIA2 = fabia_result[["scores"]][["scores"]][["score_FABIA1"]],
            score_MOFA2 = mofa_result[["scores"]][["scores"]][["score_MOFA1"]],
            score_MFA2 = mfa_result[["scores"]][["factor_scores"]][["score_MFA2"]],
            score_GFA2 = gfa_result[["scores"]][["factor_scores"]][["score_GFA2"]]
          )
          ...
          SampleData2 <- SampleData2 %>% arrange(as.numeric(ID.x))
...
sigmas <- 5  # Define sigma or the range of sigmas
