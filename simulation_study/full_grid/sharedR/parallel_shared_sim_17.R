    ...
    if (method == "single.factor") {
      ...
      for (i in 1:length(simulated_data)) {
        try({
          ...
          indices_features.1c <- simulated_data[[paste0("iteration_", i)]]$indices_features.1
          indices_features.2c <- simulated_data[[paste0("iteration_", i)]]$indices_features.2
          indices_samples.1c <- simulated_data[[paste0("iteration_", i)]]$indices_samples.1
          indices_samples.2c <- simulated_data[[paste0("iteration_", i)]]$indices_samples.2
            ...
            if (varphi_function == "varphi.five"){
              Data_Smp2 <- Data_Smp %>%
                mutate(
                  fabia_index = ifelse(
                    abs(score_FABIA) >= do.call(get(varphi_function), list(
                      loading_or_score = score_FABIA, type = "score"
                    )), 1, 0),
                  
                  mofa_index = ifelse(
                    abs(score_MOFA) >= do.call(get(varphi_function), list(
                      loading_or_score = score_MOFA, type = "score"
                    )), 1, 0),
                  
                  mfa_index = ifelse(
                    abs(score_MFA) >= do.call(get(varphi_function), list(
                      loading_or_score = score_MFA, type = "score"
                    )), 1, 0),
                  
                  gfa_index = ifelse(
                    abs(score_GFA) >= do.call(get(varphi_function), list(
                      loading_or_score = score_GFA, type = "score"
                    )), 1, 0),
...
methods <- c('shared.factor')#'single.factor', 'shared.factor', 'random.shared.factor'
sigmas <- 17  # Define sigma or the range of sigmas
